...
function Sport(props) {
  ...
  let { dataList, totalPages, handlePageClick, handleShowEnties, noOfRecords, handleBetType, betType, handleRadio, optionRadioName } = props;

  // let { dataList } = props;

  // console.log("MODAL____currentBetsItems::", dataList);
  console.log("MODAL____totalPages::", totalPages);
      ...
      <div className='w-full bg-white  lg:flex justify-between md:space-y-0 space-y-4 pb-4'>
        <div className='md:flex md:space-x-6 md:space-y-0 space-y-4 '>
          <div className="flex  md:space-x-10 space-x-4 ">
            ...
            <div className='flex space-x-4 items-center xl:text-sm text-xs'>
              <label className=' font-medium text-slate-600 cursor-pointer flex space-x-2 '><input name="rtwo" checked={betType && betType === 1 ? true : false} type='radio' onClick={() => handleBetType(1)} /><span>All</span></label>
              <label className=' font-medium text-slate-600 cursor-pointer flex space-x-2 '><input name="rtwo" checked={betType && betType === 2 ? true : false} type='radio' onClick={() => handleBetType(2)} /><span>Back</span></label>
              <label className=' font-medium text-slate-600 cursor-pointer flex space-x-2 '><input name="rtwo" checked={betType && betType === 3 ? true : false} type='radio' onClick={() => handleBetType(3)} /><span>Lay</span></label>
            </div>
          </div>
          ...
          <div className="inline-block min-w-full ">
            <div className="overflow-hidden w-full ">
              ...
              <table className="min-w-full capitalize border border-collapse ">
                ...
                <tbody className='divide-y divide-white border-b'>

                  {
                    dataList && dataList.length > 0 ?
                      dataList.map((element, index) => (
                        <React.Fragment key={index}>
                          <tr className="text-[#495057] text-[12px] bg-white odd:bg-sky-100 even:bg-pink-200/80   ">
                  ...
                  {/* <tr className="text-gray-800 text-sm bg-white">
                    <td className="p-4 whitespace-nowrap">{element && element.eventType ? element.eventType : "-"}</td>
                    <td className="p-4 whitespace-nowrap">{element && element.eventNamem ? element.eventNamem : "-"}</td>
                    <td className="p-4 whitespace-nowrap">{element && element.username ? element.username : "-"}</td>
                    <td className="p-4 whitespace-nowrap">{element && element.nation ? element.nation : "-"}</td>
                    <td className="p-4 whitespace-nowrap">{element && element.price ? element.price : "-"}</td>
                    <td className="p-4 whitespace-nowrap">{element && element.amount ? element.amount : "-"}</td>
                    <td className="p-4 whitespace-nowrap">{element && element.marketname ? element.marketname : "-"}</td>
                    <td className="px-2 whitespace-nowrap">
                      <div className='flex items-center justify-left p-1 '>

                        <div className='flex rounded overflow-hidden'>
                          <button className=' text-white text-sm px-3 py-1.5 bg-[#34c38f]'
                          >D  </button>
                        </div>
                      </div>
                    </td>
