...
export default function CreateAddCategoryModal(props) {
  ...
  let { handleAddBannerHideModal, inputAddBannerChange,
    ...
    fieldsBanner,getAllQuotesSubCategory,
    errorsBanner,
    createBannerSubmit,
  } = props;
  return (
        ...
        <div className="p-4">
          ...
          <form autoComplete="off" className="p-4 space-y-4 capitalize border border-gray-300 ">
            ...
            <div className="w-full">
              ...
              <select class="form-select border-1 px-3 py-2 placeholder-blueGray-400 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" aria-label="Default select example" onChange={inputAddBannerChange} id="bannerCategoryId" name="bannerCategoryId"
                value={fieldsBanner && fieldsBanner["bannerCategoryId"] ? fieldsBanner["bannerCategoryId"] : null} >
                <option selected>Please Select Quotes </option>
                {
                  getAllQuotesSubCategory && getAllQuotesSubCategory && getAllQuotesSubCategory.length > 0 ?
                    getAllQuotesSubCategory.map((element, index) => (
                      <option value={element && element.id ? element.id : null}>{element && element.name ? element.name : "NA"}</option>
                    )) : null
                }
              </select>
              {errorsBanner && errorsBanner["bannerCategoryId"] ?
                <div className="text-xs text-red-600 invalid-feedback">
                  {errorsBanner["bannerCategoryId"]}
                </div>
                : null}
            </div>
